...
//find total task of specific users
router.get('/totalTask', async(req, res) => {

        try {
            const totaltask = await Task.aggregate([{ $group: { _id: "$owner", totalTask: { $sum: 1 } } }])
            res.status(200).send(totaltask)
        } catch (e) {
            res.status(500).send()
        }
    })
